...
zoomBackend :: (CB.XBackend s q -> CB.XBackendM s q) -> State (RawCalendar s q) ()
...
doCalendar :: Monad m => (forall s q. -- (Eq i, Hashable i) =>
...
cachedIncarnations (Calendar c) range =
    runReader (CB.cachedIncarnations (_calBackend c) range) (_calState c)
